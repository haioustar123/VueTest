    ...
    export default {
        data () {
           ...
           <% domainClass.gormPersistentEntity.persistentProperties.eachWithIndex { property,index->
                ...
                if(index>7){return}
                if(property.type!=java.util.SortedSet && property.type!=java.util.Set && property.type!=([] as Byte[]).class && property.type!=([] as byte[]).class && property.type!=java.sql.Blob){%>
                    {title: '${propertiesZhAnnotation[property.name]}', key: '${property.name}'}<%if((index+1)!=propertySize){%>,<%}%>   <%}%>    <%}%>
                ],
                data: [],
                selectedData: [],
                rowHandle: {
       ...
       <%persistentProps.eachWithIndex { property,index->
               if(excludedProps.contains(property.name)){return}
               def cp=constrainedProperties[property.name];
               String required="required";
               if(cp?.nullable || cp?.blank){required="";}
             ...
             <%}else if (property.type == URL){
               ...
               }else if (property.type==([] as Byte[]).class || property.type==([] as byte[]).class || property.type==java.sql.Blob){
