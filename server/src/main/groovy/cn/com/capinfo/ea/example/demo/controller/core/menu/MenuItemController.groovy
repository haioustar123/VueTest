package cn.com.capinfo.ea.example.demo.controller.core.menu

import cn.com.capinfo.ea.example.demo.domain.core.BaseRole
import cn.com.capinfo.ea.example.demo.domain.core.Requestmap
import cn.com.capinfo.ea.example.demo.domain.core.menu.MenuItem
import cn.com.capinfo.ea.example.demo.service.core.UtilService
import cn.com.capinfo.ea.example.demo.service.core.menu.MenuItemService
...
class MenuItemController {
    ...
    public Map save(MenuItem menuItemInstance) {
        ...
        if (!menuItemService.save(menuItemInstance)) {
            ...
            map.result = true;
            String domainName = CapinfoSpringUtils.getI18nMessage("menuItem.label");
            map.message = CapinfoSpringUtils.getI18nMessage("default.created.message", [domainName, menuItemInstance.id]);
        }
        return map;
    }
    ...
    public Map update(MenuItem menuItemInstance, Long version) {
        ...
        if (version != null) {
            if (menuItemInstance.version > version) {
                map.result = false
                map.message = CapinfoSpringUtils.getI18nMessage("default.optimistic.locking.failure", [menuItemInstance.id]);
                return map;
            }
        }
        if (menuItemInstance.hasErrors()) {
        ...
        if (!result) {
            ...
            map.result = true;
            String domainName = CapinfoSpringUtils.getI18nMessage("menuItem.label");
            map.message = CapinfoSpringUtils.getI18nMessage("default.updated.message", [domainName, menuItemInstance.id]);
        }
        return map;
    }
