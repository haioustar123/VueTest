...
import cn.com.capinfo.ea.plugin.springsecurity.captcha.CaptchaVerificationFailedException
...
class LoginController {
    ...
    public void concurrentSession(HttpServletRequest request, HttpServletResponse response, Model model) {
        String msg = CapinfoSpringUtils.getI18nMessage("springSecurity.errors.login.concurrent.session");
        response.setHeader("Content-type", "text/html;charset=UTF-8");
        if (CapinfoSpringSecurityUtils.isAjax(request)) {
    ...
    public void alreadyLogin(HttpServletRequest request, HttpServletResponse response) {
        String url = request.contextPath + CapinfoSpringUtils.getConfiginfo("capinfo.springsecurity.successHandler.defaultTargetUrl");
        String msg = CapinfoSpringUtils.getI18nMessage("springSecurity.errors.login.already", [url]);
        response.setHeader("Content-type", "text/html;charset=UTF-8");
        if (CapinfoSpringSecurityUtils.isAjax(request)) {
    ...
    public String auth(HttpServletRequest request, HttpServletResponse response, Model model) {
        if (capinfoSpringSecurityService.isLoggedIn()) {
            ...
            if (capinfoSpringSecurityService.isAjax(request)) {
                response.writer.println("{result:false,message:'need login'}")
                return null
            }
            nocache(response)
            String postUrl = "${request.contextPath}${CapinfoSpringUtils.getConfiginfo('capinfo.springsecurity.apf.filterProcessesUrl')}"
    ...
    public Map ajaxSuccess() {
        Map map = [:];
        map.result = true;
        map.url = CapinfoSpringUtils.getConfiginfo("capinfo.springsecurity.successHandler.defaultTargetUrl");
        return map;
    }
    ...
    public Map ajaxDenied(javax.servlet.http.HttpServletRequest request, javax.servlet.http.HttpServletResponse response) {
        ...
        Map map = [:];
        map.result = false;
        map.message = CapinfoSpringUtils.getI18nMessage("springSecurity.denied.message");
        return map;
    }
