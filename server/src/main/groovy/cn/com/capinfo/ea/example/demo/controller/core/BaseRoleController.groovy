...
class BaseRoleController {
    ...
    public Map save(BaseRole baseRoleInstance, Model model) {
        ...
        if (!baseRoleService.save(baseRoleInstance)) {
            ...
            map.message = CapinfoSpringUtils.getI18nMessage("default.created.message", [domainName, baseRoleInstance.id]);
        }
        return map;
    }
    ...
    public Map update(BaseRole baseRoleInstance, Long version) {
        ...
        if (version != null) {
            if (baseRoleInstance.version > version) {
                map.result = false
                map.message = CapinfoSpringUtils.getI18nMessage("default.optimistic.locking.failure", [baseRoleInstance.id]);
                return map;
            }
        }
        if (baseRoleInstance.hasErrors()) {
        ...
        if (!result) {
            ...
            map.result = true;
            String domainName = CapinfoSpringUtils.getI18nMessage("baseRole.label");
            map.message = CapinfoSpringUtils.getI18nMessage("default.updated.message", [domainName, baseRoleInstance.id]);
        }
        return map;
    }
